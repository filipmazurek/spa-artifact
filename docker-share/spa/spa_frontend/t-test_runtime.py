...
from smc.util.two_sample_t_test import modified_threshold_two_sample_t_test
...
class TTestRuntime:
    ...
    def single_direction_search(self, starting_point: _T_NUMBER, direction_operator):
        ...
        while True:
            ...
            T, t, proportion_comparison_result, result_for_confidence, conf = modified_threshold_two_sample_t_test(self.data_generators[0].get_data_list(), param, self.proportion, 1-self.confidence)

            if conf > self.confidence:
                res = True
            elif conf < 1-self.confidence:
                res = False
            else:
                res = None
...
def run(property_: _T_PROPERTY, data_generators: list[DataGenerator], proportion: float = 0.9, confidence: float = 0.9,
    ...
    spa = TTestRuntime(property_=property_, proportion=proportion, confidence=confidence, data_generators=data_generators,
